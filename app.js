...
const showImages = (images,length) => {
  ...
  if(length === 0){
        ...
        const alertMessage = `
            <h1 class="alertMessage"> SORRY WE CAN'T FOUND ANYTHING </h1>
        `;
...
const createSlider = () => {
  ...
  if( duration < 0 ){
    alert("You can't put negative value,your duration now fixed in 1 sec !!");
    duration = 1000;
  }
...
// ENTER BUTTON FOR CREATE SLIDER
const searchField = document.getElementById('duration');
...
searchBtn.addEventListener('click', function () {
  ...
  if(search.value==0){
    ...
    alertDiv.className = 'alert';
    const alertMessage = `
        <h1 class="alertMessage"> YOU CAN TYPE ANYTHING WHAT YOU WANT !!!!</h1>
    `;
    alertDiv.innerHTML = alertMessage;
    alertContainer.style.display = 'block';
    alertContainer.appendChild(alertDiv);
...
// toggle spinner for loading animation
